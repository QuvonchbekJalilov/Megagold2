...
<x-layouts.main2>
                            ...
                            <div class="widget-content">
                                <ul class="category-list clearfix">
                                    @foreach($categories as $tm)
                                        <li><a href="{{ route('products', ['product' => $tm->id])}}"
                                               style="color: white;">{{$tm['name_'. $lang]}}</a>
                                        </li>
                                    @endforeach
                    ...
                    <div class="single_price d-flex align-items-center justify-content-flex-end gap-5 wow animate__fadeIn shadow px-3  "
                        ...
                        <div class="choose_plan">
                            <button class="cmn_btn increase_width cmn_outline effect px-5 py-2 rounded"
                                ...
                                <span>  <span>{{__('words.buy')}}</span></span>
                            </button>
                        </div>
                    </div>
                    @endforeach
    ...
    <div class="modal fade" style="margin-top: 90px;" id="purchaseModal" tabindex="-1"
        ...
        <div class="modal-dialog">
            <div class="modal-content" style="padding:10px; border-radius:1.3rem;">
                ...
                <div class="modal-body">
                    ...
                    <form id="purchaseForm" action="{{ route('orders.store')}}" method="post"
                          onsubmit="return confirm('Maxsulotni o\'zlashtirishga ruxsat berasizmi')">
                        @csrf
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="name" class="form-control" id="fullName"
                                           placeholder="{{__('words.placeholderName')}}" required>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" name="phone_number" id="phoneNumber"
                                           required placeholder="+998" :value="+998">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="weight" class="form-label">{{__('words.weight')}} (kg):</label>
                                    <input type="text" class="form-control" name="total_weight" id="weight"
                                           placeholder="{{__('words.placeholderWeight')}}"
                                           oninput="calculateFromWeight()" required>
                                </div>

                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="weight" disabled class="form-label">{{__('words.lenght')}}
                                        (Metr):</label>
                                    <input type="text" class="form-control" name="total_length" id="length"
                                           placeholder="{{__('words.placeholderLenght')}}"
                                           oninput="calculateFromLength()" required>
                                </div>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label for="price" class="form-label">{{__('words.cost')}} (UZS):</label>
                            <input type="text" class="form-control" name="total_price" id="price" placeholder=""
                                   readonly required>
                        ...
                        <button type="submit" onclick="sendOrderInfo()"
                                class="btn btn-primary mt-3">{{__('words.submit')}}</button>
                    </form>
                </div>
            </div>
        </div>
